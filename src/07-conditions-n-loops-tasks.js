...
function findFirstSingleChar(str) {
  ...
  return str.split('').find((item) => obj[item] === 1);
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = str.split('').reverse().map((item, index) => {
    if (index % 2 === 0) {
      return String(item * 2).split('').reduce((akk, it) => +akk + +it);
    }
    return item;
  }).reduce((akk, item) => +akk + +item);
  let last = 10 - (sum % 10);
  if (last === 10) last = 0;
  if (last === +String(ccn).slice(-1)) return true;
  return false;
}
...
function getDigitalRoot(num) {
  ...
  const newNum = str.split('').reduce((akk, it) => +akk + +it).toString();
  if (newNum.length < 2) {
    return +newNum;
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i <= newPathes[0].length; i += 1) {
    if (newPathes.every((item) => item.includes(newPathes[0][i]))) arr.push(newPathes[0][i]);
  }
